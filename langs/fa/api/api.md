...
رویکرد کلی Solid به واکنش پذیری این است که هر محاسبات واکنشی را در یک تابع بپیچد و زمانی که وابستگی های آن به روز می شود، آن تابع را مجددا اجرا می کند. کامپایلر Solid JSX همچنین اکثر عبارات JSX (کد در پرانتز) را با یک تابع می‌پیچد، بنابراین وقتی وابستگی‌هایشان تغییر می‌کند، به‌طور خودکار به‌روزرسانی می‌شوند (و به‌روزرسانی‌های DOM مربوطه را راه‌اندازی می‌کنند). به طور دقیق تر، اجرای مجدد خودکار یک تابع هر زمان که تابع در یک محدوده ردیابی فراخوانی شود، مانند عبارت JSX یا فراخوانی های API که «محاسبات» را ایجاد می کنند (createEffect، createMemo و غیره) اتفاق می افتد. به طور پیش‌فرض، وابستگی‌های یک تابع زمانی که در محدوده ردیابی فراخوانی می‌شوند، به‌طور خودکار ردیابی می‌شوند، با تشخیص زمانی که تابع حالت واکنشی را می‌خواند (مثلاً از طریق یک سیگنال گیرنده یا ویژگی Store). در نتیجه، شما به طور کلی نیازی به نگرانی در مورد وابستگی خود ندارید. (اما اگر ردیابی وابستگی خودکار هرگز نتایجی را که می‌خواهید ایجاد نکرد، می‌توانید ردیابی وابستگی را لغو کنید.) این رویکرد واکنش‌پذیری را قابل ترکیب می‌کند: فراخوانی یک تابع در تابع دیگر عموماً باعث می‌شود که تابع فراخوانی وابستگی‌های تابع فراخوانی شده را به ارث ببرد.
